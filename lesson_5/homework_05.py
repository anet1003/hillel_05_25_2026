...
#for year, engine_volume, price in car_data:
...
for car_name, car_info in car_data.items():
    if(
        car_info[1] >= search_criteria[0]
        and car_info[2] >= search_criteria[1]
            and car_info[4] <= search_criteria[2]
    ):
        result.append((car_name, car_info))

result.sort(key=lambda x: x[1][4])
for car in result[:5]:
    print(car[0], car[1])
